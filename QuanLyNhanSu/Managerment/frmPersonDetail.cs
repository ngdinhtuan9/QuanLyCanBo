...
namespace QuanLyNhanSu.Category
{
    public partial class frmPersonDetail : Form
    {
               ...
               // dteYearBegin.CustomFormat = " ";
                ...
                if (person != null)
                {
                   ...
                   // string[] items = person.UnitCode.Split('-');
                    ...
                    cbxSchoolName.Text =  person.SchoolName + QLNSCommon.delimiter + person.Major + QLNSCommon.delimiter + person.AcademicLevelName;
                    cbxInternalSchool.Text =  person.InternalSchool + QLNSCommon.delimiter + person.InternalMajor + QLNSCommon.delimiter + person.InternalAcademicLevelName;
                    cbxCurInternalSchool.Text =  person.CurInternalSchool + QLNSCommon.delimiter + person.CurInternalMajor + QLNSCommon.delimiter + person.CurInternalAcademicLevelName;
        ...
        private void save(bool savebtn)
        {
            ...
            {
                List<string> result = QLNSCommon.SplitStringByDelimiter(cbxSchoolName.Text);
            ...
            {
                List<string> result = QLNSCommon.SplitStringByDelimiter(cbxInternalSchool.Text); //new List<string>(cbxInternalSchool.Text.Split('-'));
            ...
            {
                List<string> result = QLNSCommon.SplitStringByDelimiter(cbxCurInternalSchool.Text);//new List<string>(cbxCurInternalSchool.Text.Split('-'));
        ...
        private bool valid()
        {
            try
            {
                ...
                //{
                //    MessageBox.Show("Bạn chưa nhập CCCD.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                //    txtIdentityCard.Focus();
                //    return false;
                //};
                //if (txtIdentityCard.Text.Trim() != "" && txtIdentityCard.Text.Trim().Length != 12)
                //{
                //    MessageBox.Show("Số căn cước công dân phải đủ 12 ký tự.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                //    txtIdentityCard.Focus();
                //    return false;
                //    //return false;
                //};
        ...
        private void EditTranningProcess(string value, string schoolName, string major, string level)
        {
            TranningProcess tranningProcess = allTranningProcess.FirstOrDefault(obj => obj.PersonCode == value && obj.SchoolName == schoolName
            && obj.Major == major && obj.Level == level);
            frmTraningProcessDetails frmDetail = new frmTraningProcessDetails();
            frmDetail.allTranningProcess = allTranningProcess;
            frmDetail.personCode = person.Code;
        ...
        private void EditFosteringProcess(string value,string schoolName,string fosteringContent, string level)
        {
            FosteringProcess fosteringProcess = allFosteringProcess.FirstOrDefault(obj => obj.PersonCode == value && obj.SchoolName == schoolName
            && obj.FosteringContent == fosteringContent && obj.Level == level);
            frmFosteringProcessDetails frmDetail = new frmFosteringProcessDetails();
            frmDetail.allFosteringProcess = allFosteringProcess;
            frmDetail.personCode = person.Code;
        ...
        private void btnEditTranningProcess_Click(object sender, EventArgs e)
        {
            try
            {
                if (dataGridView1.SelectedRows.Count > 0)
                {
                    ...
                    string cellValue = Checking.CheckNullString(selectedRow.Cells["PersonCode"].Value);
                    string SchoolName = Checking.CheckNullString((selectedRow.Cells["SchoolName"].Value));
                    string Major = Checking.CheckNullString((selectedRow.Cells["Major"].Value));
                    string Level = Checking.CheckNullString((selectedRow.Cells["Level"].Value));
                    EditTranningProcess(cellValue, SchoolName, Major, Level);
                }
                else
                {
        ...
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                string value = Checking.CheckNullString((clickedRow.Cells["PersonCode"].Value));
                string SchoolName = Checking.CheckNullString((clickedRow.Cells["SchoolName"].Value));
                string Major = Checking.CheckNullString((clickedRow.Cells["Major"].Value));
                string Level = Checking.CheckNullString((clickedRow.Cells["Level"].Value));
                EditTranningProcess(value, SchoolName, Major, Level);
            }
            catch (Exception ex)
            {
                MessageBox.Show("Lỗi trong quá trình xử lý dữ liệu.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void dataGridView2_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                string value = Checking.CheckNullString((clickedRow.Cells["PersonCode"].Value));
                string SchoolName = Checking.CheckNullString((clickedRow.Cells["SchoolName"].Value));
                string FosteringContent = Checking.CheckNullString((clickedRow.Cells["FosteringContent"].Value));
                string Level = Checking.CheckNullString((clickedRow.Cells["Level"].Value));
                
                EditFosteringProcess(value, SchoolName, FosteringContent, Level);
            }
            catch (Exception ex)
            {
                MessageBox.Show("Lỗi trong quá trình xử lý dữ liệu.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btnEditFosteringProcess_Click(object sender, EventArgs e)
        {
            try
            {
                if (dataGridView2.SelectedRows.Count > 0)
                {
                    ...
                    string cellValue = Checking.CheckNullString(selectedRow.Cells["PersonCode"].Value);
                    string SchoolName = Checking.CheckNullString(selectedRow.Cells["SchoolName"].Value);
                    string FosteringContent = Checking.CheckNullString(selectedRow.Cells["FosteringContent"].Value);
                    string Level = Checking.CheckNullString(selectedRow.Cells["Level"].Value);
                    EditFosteringProcess(cellValue, SchoolName, FosteringContent, Level);
                }
                else
                {
                    MessageBox.Show("Chưa có thông tin nào được chọn.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void cbxSchoolName_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                List<string> result = QLNSCommon.SplitStringByDelimiter(cbxSchoolName.Text);//new List<string>(cbxSchoolName.Text.Split('-'));
