    ...
    public partial class frmPerson : Form
    {
        ...
        private void Search()
        {
            try
            {
                ...
                if (cbxType.Text.Trim() != "" && cbxType.Text.Trim() != "Tất cả")
                {
                    if (cbxType.Text != "Tất cả")
                    {
                        ...
                        {
                            personSearch = personSearch.FindAll(item => item.InternalAcademicLevelName != "");
                        }
                        else if (cbxType.Text == "Tốt nghiệp trường ngoài CAND")
                        {
                            personSearch = personSearch.FindAll(item => item.AcademicLevelName != "");
                        }
                        else if (cbxType.Text == "Đang học các lớp trong CAND")
                        {
                            personSearch = personSearch.FindAll(item => item.CurInternalAcademicLevelName != "");
                        }
                    }
                    //else
                    //{
        ...
        private void Resize()
        {
            ...
            if (dataGridView1.Columns["id"] != null)
            {
                ...
                dataGridView1.Columns["IdentityCard"].Frozen = true;
                ...
                for (int rowIndex = 3; rowIndex < worksheet.Rows.Count; rowIndex++)
                {
                   ...
                   // string depart = "";
                    ...
                    Person data = new Person
                    {
                        ...
                        IdentityCard = Checking.CheckNullString(row.Cells[2].Value),
                        FullName = Checking.CheckNullString(row.Cells[3].Value),
                        DateOfBird = Checking.CheckNullString(row.Cells[4].Value),
                        Sex = Checking.CheckNullString(row.Cells[5].Value),
                        Hometown = Checking.CheckNullString(row.Cells[6].Value),
                        Address = Checking.CheckNullString(row.Cells[7].Value),
                        ResponsibleName = Checking.CheckNullString(row.Cells[8].Value),
                        YearBegin = Checking.CheckNullString(row.Cells[9].Value),
                        OfficialStartYear = Checking.CheckNullString(row.Cells[10].Value),
                        CommunistPartyBegin = Checking.CheckNullString(row.Cells[11].Value),
                        LevelCode = Checking.CheckNullString(row.Cells[12].Value),
                        Position = Checking.CheckNullString(row.Cells[13].Value),
                        Tittle = Checking.CheckNullString(row.Cells[14].Value),

                        InternalSchool = Checking.CheckNullString(row.Cells[15].Value),
                        InternalMajor = Checking.CheckNullString(row.Cells[16].Value),
                        InternalSchoolYear = Checking.CheckNullString(row.Cells[17].Value),
                        InternalAcademicLevelName = Checking.CheckNullString(row.Cells[18].Value),
                        InternalTraningType = Checking.CheckNullString(row.Cells[19].Value),

                        SchoolName = Checking.CheckNullString(row.Cells[20].Value),
                        Major = Checking.CheckNullString(row.Cells[21].Value),
                        SchoolYear = Checking.CheckNullString(row.Cells[22].Value),
                        AcademicLevelName = Checking.CheckNullString(row.Cells[23].Value),
                        TraningType = Checking.CheckNullString(row.Cells[24].Value),

                        CurInternalSchool = Checking.CheckNullString(row.Cells[25].Value),
                        CurInternalMajor = Checking.CheckNullString(row.Cells[26].Value),
                        CurInternalSchoolYear = Checking.CheckNullString(row.Cells[27].Value),
                        CurInternalAcademicLevelName = Checking.CheckNullString(row.Cells[28].Value),
                        CurInternalTraningType = Checking.CheckNullString(row.Cells[29].Value),

                        FosteringPLNV = Checking.CheckNullString(row.Cells[30].Value),
                        TittleTraning = Checking.CheckNullString(row.Cells[31].Value),
                        TittleTraningPlanning = Checking.CheckNullString(row.Cells[32].Value),
                        FosteringQPA = Checking.CheckNullString(row.Cells[33].Value),
                        PoliticalLevel = Checking.CheckNullString(row.Cells[34].Value),
                        PoliticalLevelForm = Checking.CheckNullString(row.Cells[35].Value),
                        Classify = Checking.CheckNullString(row.Cells[36].Value),
                        Competition = Checking.CheckNullString(row.Cells[37].Value),
                        UnitCode = Checking.CheckNullString(row.Cells[38].Value),
                        Department = Checking.CheckNullString(row.Cells[39].Value),
                        Note = Checking.CheckNullString(row.Cells[40].Value)
                    };
                     message = string.Empty;
        ...
        private bool valid(Person personValid, ref string message)
        {
            try
            {
                ...
                if (Checking.CheckNullString(personValid.Department) != "")
                {
                    //neu chu co trong danh muc thi them vao
                    //List<Department> depSearch = allDepartment.FindAll(item => item.UnitId == Checking.CheckNullInt(personValid.Code));
                    Unit unitSearch = allUnits.FirstOrDefault(item => item.Name == Checking.CheckNullString(personValid.UnitCode));
                    List<Department> depSearch = allDepartment.FindAll(item => item.UnitId == unitSearch.Id && 
                    item.Name == personValid.Department);
                    if (depSearch.Count == 0)
                    {
                        
                        Department depAll = new Department();
                        depAll.Name = personValid.Department;
                        depAll.UnitId = unitSearch.Id;
                        depAll.Id = 99999;
                        allDepartment.Add(depAll);
                        string str = Newtonsoft.Json.JsonConvert.SerializeObject(allDepartment);
                        Common.SaveFileContent(QLNSCommon.pathCategory + fileDepartmentName, str);
                    }
                }
