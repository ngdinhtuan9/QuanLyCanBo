...
using QuanLyNhanSu.HuanLuyenDauKhoa;
    ...
    public partial class frmMainNew : Form
    {
        ...
        private void mnuLogin_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (frmLogin.isLogin)
                {
                    if (frmLogin.function == 100 || frmLogin.function == 1 || frmLogin.function == 0)
                    {
                        ...
                        else if (frmLogin.role == "Người dùng")
                        {
                            ...
                            mnuLevelReport.Visible = false;
                        }
                        if (frmLogin.function == 100)
                        {
                            mnuQLHL.Visible = true;
                        }

                    }
                    else if (frmLogin.function == 2)
                    {
                        mnuQLHL.Visible = true;
                    }
        ...
        private void frmMainNew_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    ...
                    QLNSCommon.glbImage = root.SelectSingleNode("Image")?.InnerText ?? "";
                ...
                if (QLNSCommon.glbImage != "")
                {
                    try
                    {
                        string imagePath = AppDomain.CurrentDomain.BaseDirectory + "\\Pic\\" + QLNSCommon.glbImage;
        ...
        private void mnuThongKe_Click(object sender, EventArgs e)
        {
            try
            {
                frmLevelReportNew frmDep = new frmLevelReportNew();
                frmDep.TopLevel = false;
                pictureBox3.Controls.Add(frmDep);
                frmDep.Location = new Point(
                   (pictureBox3.Width - frmDep.Width) / 2,
        ...
        private void mnuQLDiem_Click(object sender, EventArgs e)
        {
            try
            {
                frmDanhSachHuanLuyen frmDep = new frmDanhSachHuanLuyen();
                frmDep.TopLevel = false;
                pictureBox3.Controls.Add(frmDep);
                frmDep.Location = new Point(
                   (pictureBox3.Width - frmDep.Width) / 2,
