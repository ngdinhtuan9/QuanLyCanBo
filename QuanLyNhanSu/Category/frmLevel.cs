    ...
    public partial class frmLevel : Form
    {
        ...
        private void Search()
        {
            try
            {
                ...
                allLevel = Newtonsoft.Json.JsonConvert.DeserializeObject<List<Level>>(content);
                dataGridView1.DataSource = allLevel;
                Resize();
            }
            catch (Exception ex)
            {
        ...
        private void Resize()
        {
            ...
            dataGridView1.Columns["Name"].HeaderText = "Tên cấp bậc";
            dataGridView1.Columns["Code"].HeaderText = "Mã cấp bậc";
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    level = allLevel.FindAll(item => item.Code.StartsWith(txtUnitCode.Text.Trim()) && item.Name.StartsWith(txtUnitName.Text.Trim()));
                }
                else if (txtUnitCode.Text.Trim() != "")
                {
                    level = allLevel.FindAll(item => item.Code.StartsWith(txtUnitCode.Text.Trim()));
                }
                else if (txtUnitName.Text.Trim() != "")
                {
                    level = allLevel.FindAll(item => item.Name.StartsWith(txtUnitName.Text.Trim()));
                }
                else
                {
                    level = allLevel;
                ...
                dataGridView1.DataSource = level;
                Resize();
            }
            catch (Exception ex)
            {
        ...
        {
            Level level = allLevel.FirstOrDefault(obj => obj.Id == value);
            frmLevelDetail frmDetail = new frmLevelDetail();
            frmDetail.levelId = level.Id;
            frmDetail.level = level;
            frmDetail.ShowDialog();
            if (frmDetail.succesed)
            {
                if (frmDetail.levelId == level.Id)
                {
                    string str = Newtonsoft.Json.JsonConvert.SerializeObject(allLevel);
                ...
                if (MessageBox.Show("Bạn có chắc chắn muốn xóa các dữ liệu đã chọn?","Xác nhận", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    for (int i = dataGridView1.Rows.Count - 1; i >= 0; i--)
                    {
                        ...
                        if (isChecked)
                        {
                            ...
                            allLevel.RemoveAll(obj => obj.Id == targetID);
                        }
                    }
                    string str = Newtonsoft.Json.JsonConvert.SerializeObject(allLevel);
                    Common.SaveFileContent(QLNSCommon.pathCategory + fileName, str);
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                int maxID = 0;
                if (allLevel != null && allLevel.Count >0) { maxID = allLevel.Max(obj => obj.Id); }
                else
                {
                    maxID = 1;
                ...
                if (frmDetail.succesed)
                {
                    if (frmDetail.levelId > 0)
                    {
                        allLevel.Add(frmDetail.level);
                        string str = Newtonsoft.Json.JsonConvert.SerializeObject(allLevel);
                        Common.SaveFileContent(QLNSCommon.pathCategory + fileName, str);
                        MessageBox.Show("Lưu dữ liệu thành công.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        dataGridView1.DataSource = null;
