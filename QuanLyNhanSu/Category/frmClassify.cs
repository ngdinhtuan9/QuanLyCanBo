...
namespace QuanLyNhanSu.Category
{
    public partial class frmClassify : Form
    {
        ...
        string fileName = "Classify\\Classify.txt";
        ...
        private void Search()
        {
            try
            {
                ...
                allClassify = Newtonsoft.Json.JsonConvert.DeserializeObject<List<Classify>>(content);
                dataGridView1.DataSource = allClassify;
                Resize();
            }
            catch (Exception ex)
            {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    Classify = allClassify.FindAll(item => item.Code.StartsWith(txtUnitCode.Text.Trim()) && item.Name.StartsWith(txtUnitName.Text.Trim()));
                }
                else if (txtUnitCode.Text.Trim() != "")
                {
                    Classify = allClassify.FindAll(item => item.Code.StartsWith(txtUnitCode.Text.Trim()));
                }
                else if (txtUnitName.Text.Trim() != "")
                {
                    Classify = allClassify.FindAll(item => item.Name.StartsWith(txtUnitName.Text.Trim()));
                }
                else
                {
                ...
                dataGridView1.DataSource = Classify;
                Resize();
            }
            catch (Exception ex)
            {
        ...
        {
            Classify Classify = allClassify.FirstOrDefault(obj => obj.Id == value);
            frmClassifyDetail frmDetail = new frmClassifyDetail();
            frmDetail.classifyId = Classify.Id;
            frmDetail.classify = Classify;
            frmDetail.ShowDialog();
            if (frmDetail.succesed)
            {
                if (frmDetail.classifyId == Classify.Id)
                {
                    string str = Newtonsoft.Json.JsonConvert.SerializeObject(allClassify);
                ...
                if (MessageBox.Show("Bạn có chắc chắn muốn xóa các dữ liệu đã chọn?","Xác nhận", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    for (int i = dataGridView1.Rows.Count - 1; i >= 0; i--)
                    {
                        ...
                        if (isChecked)
                        {
                            ...
                            allClassify.RemoveAll(obj => obj.Id == targetID);
                        }
                    }
                    string str = Newtonsoft.Json.JsonConvert.SerializeObject(allClassify);
                    Common.SaveFileContent(QLNSCommon.pathCategory + fileName, str);
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                int maxID = 0;
                if (allClassify != null && allClassify.Count >0) { maxID = allClassify.Max(obj => obj.Id); }
                else
                {
                    maxID = 1;
                     allClassify = new List<Classify>();
                ...
                frmClassifyDetail frmDetail = new frmClassifyDetail();
                frmDetail.maxClassifyId = maxID;
                frmDetail.ShowDialog();
                if (frmDetail.succesed)
                {
                    if (frmDetail.classifyId > 0)
                    {
                        allClassify.Add(frmDetail.classify);
                        string str = Newtonsoft.Json.JsonConvert.SerializeObject(allClassify);
                        Common.SaveFileContent(QLNSCommon.pathCategory + fileName, str);
                        MessageBox.Show("Lưu dữ liệu thành công.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        dataGridView1.DataSource = null;
