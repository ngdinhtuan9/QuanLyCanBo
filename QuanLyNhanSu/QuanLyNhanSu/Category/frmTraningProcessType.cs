...
{
    public partial class frmTraningProcessType : Form
    {
        List<TraningProcessType> allTraningProcessType;
        string fileName = "TraningProcessType\\TraningProcessType.txt";
         List<TraningProcessType> traningProcessType;
         

        public frmTraningProcessType()
        {
            InitializeComponent();
        }
        ...
        private void Search()
        {
            try
            {
                ...
                allTraningProcessType = Newtonsoft.Json.JsonConvert.DeserializeObject<List<TraningProcessType>>(content);
                dataGridView1.DataSource = allTraningProcessType;
                Resize();
            }
            catch (Exception ex)
            {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    traningProcessType = allTraningProcessType.FindAll(item => item.Code.StartsWith(txtUnitCode.Text.Trim()) && item.Name.StartsWith(txtUnitName.Text.Trim()));
                }
                else if (txtUnitCode.Text.Trim() != "")
                {
                    traningProcessType = allTraningProcessType.FindAll(item => item.Code.StartsWith(txtUnitCode.Text.Trim()));
                }
                else if (txtUnitName.Text.Trim() != "")
                {
                    traningProcessType = allTraningProcessType.FindAll(item => item.Name.StartsWith(txtUnitName.Text.Trim()));
                }
                else
                {
                    traningProcessType = allTraningProcessType;
                }

                dataGridView1.DataSource = traningProcessType;
                Resize();
            }
            catch (Exception ex)
            {
        ...
        {
            TraningProcessType TraningProcessType = allTraningProcessType.FirstOrDefault(obj => obj.Id == value);
            frmTraningProcessTypeDetail frmDetail = new frmTraningProcessTypeDetail();
            frmDetail.traningProcessTypeId = TraningProcessType.Id;
            frmDetail.traningProcessType = TraningProcessType;
            frmDetail.ShowDialog();
            if (frmDetail.succesed)
            {
                if (frmDetail.traningProcessTypeId == TraningProcessType.Id)
                {
                    string str = Newtonsoft.Json.JsonConvert.SerializeObject(allTraningProcessType);
                ...
                if (MessageBox.Show("Bạn có chắc chắn muốn xóa các dữ liệu đã chọn?","Xác nhận", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    for (int i = dataGridView1.Rows.Count - 1; i >= 0; i--)
                    {
                        ...
                        if (isChecked)
                        {
                            ...
                            allTraningProcessType.RemoveAll(obj => obj.Id == targetID);
                        }
                    }
                    string str = Newtonsoft.Json.JsonConvert.SerializeObject(allTraningProcessType);
                    Common.SaveFileContent(QLNSCommon.pathCategory + fileName, str);
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                int maxID = 0;
                if (allTraningProcessType != null && allTraningProcessType.Count >0) { maxID = allTraningProcessType.Max(obj => obj.Id); }
                else
                {
                    maxID = 1;
                     allTraningProcessType = new List<TraningProcessType>();
                ...
                frmTraningProcessTypeDetail frmDetail = new frmTraningProcessTypeDetail();
                frmDetail.maxTraningProcessTypeId = maxID;
                frmDetail.ShowDialog();
                if (frmDetail.succesed)
                {
                    if (frmDetail.traningProcessTypeId > 0)
                    {
                        allTraningProcessType.Add(frmDetail.traningProcessType);
                        string str = Newtonsoft.Json.JsonConvert.SerializeObject(allTraningProcessType);
                        Common.SaveFileContent(QLNSCommon.pathCategory + fileName, str);
                        MessageBox.Show("Lưu dữ liệu thành công.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        dataGridView1.DataSource = null;
